...
// async function analyzeCodeWithAI(codeLine: string, surroundingContext: string, elementType: string): Promise<any> {
...
//     };
// }
// function getElementType(position: vscode.Position, document: vscode.TextDocument): string {
//     const languageId = document.languageId;
...
//         if (functionRanges.some(isInRange)) return 'Function';
//         if (loopRanges.some(isInRange)) return 'Loop';
//         if (conditionalRanges.some(isInRange)) return 'Conditional';
//         if (switchRanges.some(isInRange)) return 'Switch Statement';
//         if (tryBlockRanges.some(isInRange)) return 'Try/Catch Block';
//         if (classRanges.some(isInRange)) return 'Class';
//         if (apiCallRanges.some(isInRange)) return 'API Call';
//         // Check for HTML/JSX elements
//         const divMatch = line.match(/<(\w+)/);
//         if (divMatch) return divMatch[1].toUpperCase() + ' Element';
//         return 'Code Block';
//     } else if (languageId === 'html') {
//         const elementMatch = line.match(/<(\w+)/);
//         if (elementMatch) return elementMatch[1].toUpperCase() + ' Element';
//         return 'HTML Content';
//     } else if (languageId === 'css') {
//         if (line.includes('{')) return 'CSS Rule';
//         if (line.includes('}')) return 'CSS Rule End';
//         return 'CSS Property';
//     }
//     return 'Code Block';
// }
...
//             const isInRange = (range: vscode.Range) => {
//                 return range.contains(position);
//             };
//             const getCodeFromRange = (range: vscode.Range): string => {
